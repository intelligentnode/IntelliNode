...
// one fake key so the provider tools resolve offline; every other key is blanked to stay deterministic
const KEYS = {
  OPENAI_API_KEY: 'sk-test', ANTHROPIC_API_KEY: '', GEMINI_API_KEY: '', MISTRAL_API_KEY: '', COHERE_API_KEY: '', NVIDIA_API_KEY: '', STABILITY_API_KEY: '',
};
...
function send(url, { method = 'POST', headers = {}, body } = {}) {
  return new Promise((resolve, reject) => {
    const target = new URL(url);
    const payload = body === undefined ? undefined : (typeof body === 'string' ? body : JSON.stringify(body));
    const request = http.request({
      host: target.hostname, port: target.port, path: target.pathname, method,
      headers: { ...headers, ...(payload !== undefined && { 'Content-Length': Buffer.byteLength(payload) }) },
    }, (response) => {
      const chunks = [];
...
async function testStdioCli() {
  ...
  assert.strictEqual(tools.length, TOOL_COUNT, 'listTools follows every cursor');
  assert.strictEqual(new Set(tools.map((tool) => tool.name)).size, TOOL_COUNT);
...
async function testHttpServer() {
  ...
  try {
    ...
    const expired = await send(url, { headers: { ...legacyHeaders, 'Mcp-Session-Id': 'gone' }, body: { jsonrpc: '2.0', id: 15, method: 'tools/list', params: {} } });
    ...
    assert.strictEqual(info.serverInfo.name, 'intellinode');
    const tools = await client.listTools();
    assert.strictEqual(tools.length, TOOL_COUNT, 'four pages are merged');
...
async function testLegacyFallback() {
  ...
  try {
    ...
    assert.deepStrictEqual(seen.map((entry) => entry.message && entry.message.method), ['server/discover', 'initialize', 'notifications/initialized']);
    ...
    const tools = await client.listTools();
    assert.deepStrictEqual(tools.map((tool) => tool.name), ['echo']);
...
function testClientHelpers() {
  ...
  assert.strictEqual(client.getTools(), client.tools);
